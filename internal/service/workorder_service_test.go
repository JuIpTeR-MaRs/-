...
// setupServiceTest initializes memory SQLite DB and memory miniredis.
// Returns a cleanup function to restore globals.
func setupServiceTest(t *testing.T) (*miniredis.Miniredis, func()) {
	// Backup original global variables
	oldDB := global.DB
	oldRedis := global.Redis

	// 1. Initialize miniredis
...
// TestGrabWorkOrderSuccess tests a normal single-user grab flow.
func TestGrabWorkOrderSuccess(t *testing.T) {
	...
	// Create test student and worker
	...
	// Verify lock is released in Redis (Lua script execution completed)
...
// TestGrabWorkOrderConcurrency tests multiple workers concurrently grabbing the same work order.
func TestGrabWorkOrderConcurrency(t *testing.T) {
	...
	// Create a single pending work order
	order := &model.WorkOrder{
		UserID:       student.ID,
		Content:      "电路故障",
	...
	// Launch concurrent grab attempts
	...
	// Wait for all grab operations to finish
	wg.Wait()
	close(successChan)
	close(errChan)
	...
	// Assert only exactly ONE worker succeeded
	if successCount != 1 {
	...
	// Fetch work order from DB to double check it is indeed assigned to the winner
...
// TestCompleteOrderWithConsumablesSuccess tests the successful flow of completing an order with consumables.
func TestCompleteOrderWithConsumablesSuccess(t *testing.T) {
	...
	// 1. Create a worker and a student
	...
	// 3. Create a work order in StatusProcessing (维修中)
	order := &model.WorkOrder{
		UserID:       student.ID,
		WorkerID:     &worker.ID,
		Content:      "更换灯管与水龙头",
	...
	// 4. Run CompleteOrderWithConsumables
	...
	// Verify consumable usage records are created
...
// TestCompleteOrderWithConsumablesFailures tests the rollback scenarios when CompleteOrderWithConsumables fails.
func TestCompleteOrderWithConsumablesFailures(t *testing.T) {
	...
	// Setup a processing work order assigned to workerA
	order := &model.WorkOrder{
		UserID:       student.ID,
		WorkerID:     &workerA.ID,
		Content:      "电路报修",
	...
	// Scenario 1: Unauthorized worker B attempts to complete worker A's order
	...
	// Verify order status and stock are unchanged
	...
	// Scenario 2: Order is not in StatusProcessing (e.g. status is StatusAssigned)
	...
	// Scenario 3: Consumable stock is insufficient (Quantity requested is 6, stock is 5)
	...
	// Verify order status and consumable stock are unchanged (successful transaction rollback)
	...
	// Scenario 4: Non-existent consumable ID
	...
	// Verify order status unchanged (successful rollback)
